...
const CarouselProject = ({ data }:{ data: typeof DATA.projects}) => {
  ...
  return (
    <div className="relative w-full  mx-auto overflow-hidden">
      <div className="relative w-full h-[500px]">
        <AnimatePresence >
          <Link target="_blank" href={`/project/${getPathFromTitle(data[currentIndex].title)}`} className="">

          <div className='w-full h-full bg-gradient-to-br from-gray-600 to-blue-800 flex flex-col rounded-sm p-4'>
            <motion.img src={`/projects/${data[currentIndex].main_image}`} alt={`Slide ${currentIndex + 1}`} className="w-full h-full object-cover rounded-lg" />
            <motion.div className="space-y-4" >
              ...
              {/* <Link target="_blank" href={`/project/${getPathFromTitle(data[currentIndex].title)}`} className="px-4 py-2 rounded-3xl bg-gray-100 text-black">
                Know more &#8594;
              </Link> */}
            </motion.div>
